...
static int PrintFrameInfo(struct mad_header *Header)
{
  ...
  ((MAD_VERSION_MAJOR==0) && (MAD_VERSION_MINOR>=15))
    ...
    //FIGHGUTSprintf("%lu kb/s audio MPEG layer %s stream at %dHz\n", Header->bitrate / 1000, Layer, Header->samplerate);
  ...
  short *_buf = (short *)_buf2;
	...
	while (samplesOut < numSamples) {
		...
		if (MAD_RECOVERABLE(Stream.error)) {
			...
			//FISHGUTSprintf("recoverable frame level error (%s)\n", MadErrorString(&Stream));
			//FIGHGUTSsceDisplayWaitVblankStart();
		    }
		    return;	//continue;
		    ...
		    //FIGHGUTSprintf("unrecoverable frame level error (%s).\n", MadErrorString(&Stream));
		    //FIGHGUTSsceDisplayWaitVblankStart();
		    //FIGHGUTSStatus = 1;
		    //FIGHGUTSMP3_Stop();	//break;
		}
	    }
...
void MP3_FreeTune()
{
	 ...
	 */
	//FIGHGUTSmad_timer_string(Timer, Buffer, "%lu:%02lu.%03u", MAD_UNITS_MINUTES, MAD_UNITS_MILLISECONDS, 0);
	//FIGHGUTSprintf("%lu frames decoded (%s).\n", FrameCount, Buffer);
	//FIGHGUTSsceDisplayWaitVblankStart();
	//FIGHGUTSsceKernelDelayThread(500000);
    }
}
